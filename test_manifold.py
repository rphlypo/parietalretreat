...
def test_random_spd():
    ...
    assert_array_less(0., linalg.eigvals(spd))
...
def test_sqrtm():
    ...
    assert_array_almost_equal(m.T, m)
...
def test_my_eigh():
    ...
    vals, vecs, success_flag = my_mfd.my_eigh(spd)
...
def test_frechet_mean():
    ...
    spds = []
    for k in xrange(n):
        spds.append(my_mfd.random_spd(shape))
    ...
    for (tol, max_iter) in [(1e-7, 3), (1e-3, 50)]:
    ...
    diags = []
    for k in xrange(n):
        diags.append(my_mfd.random_diagonal_spd(shape))
    exact_fre = np.prod(my_mfd.my_stack(diags), axis=0) ** \
                (1 / float(len(diags)))
    app_fre = my_mfd.frechet_mean(diags, tol=tol)
    assert_array_almost_equal(app_fre, exact_fre, decimal)

    # 2 matrices: TODO (why failure)
    spd1 = my_mfd.random_spd(shape)
    spd2 = my_mfd.random_spd(shape)
    exact_fre = spd1.dot(my_mfd.sqrtm(np.dot(linalg.inv(spd1), spd2)))
