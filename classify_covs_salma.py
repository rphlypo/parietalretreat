...
def statistical_test(df, conditions, estimators={'kind': 'tangent',
    ...
    for (ix1_, condition1) in enumerate(conditions):
        for (ix2_, condition2) in enumerate(conditions):
            ...
            baseline_signif = var_unstabilize(
            ...
            followup_signif = var_unstabilize(
            ...
            q[np.isnan(q)] = 0.
            comp_signif = (var_unstabilize(Y[1::2, ...], estimators['kind']) -\
...
def plot_results(t_df, save_dir, p_th=.05, estim_title=None):  # TODO threshold when plot
    if estim_title is not None:
        estim_title = " ({})".format(estim_title)
    for ix_ in range(len(t_df)):
#        tstats[pvals > p_th] = 0.
        names = ["baseline", "follow up", "comparison"]
        titles = [t_df[name].iloc[ix_] + estim_title for name in names]
        matrices = [vec_to_sym(t_df["mean signif " + name].iloc[ix_],
...
if __name__ == "__main__":
    ...
    cache_dir = os.path.join("/media/Elements/workspace/parietal_retreat/",
                             "salma", "visual_AN_DMN", "test")
    save_dir = os.path.join(cache_dir, "figures")
    n_jobs = 1
    ...
    t_df = statistical_test(root_dir=root_dir)
    plot_results(t_df, save_dir, p_th=.05, estim_title=None)